...
def main(args):
    ...
    kernel_list = []
    kernel_list = [create_density_kernel(kernel_size_list[index], sigma_list[index]) for index in
                   range(len(sigma_list))]
        ...
        for file in img_list:
            ...
            for sub_index, (image, density) in enumerate(zip(images, densities)):
                ...
                if args.with_density:
                    req_image = [(density[:, :, index] / normalizer[index] * 255.).clip(0, 255).astype(np.uint8) for
                                 index in range(len(normalizer))]
