...
DRAW_MAPS=True# If set to True, then in evaluation stage
...
class DataParameter():
    ...
    def __init__(self, model_kwargs, args) -> None:
        ...
        self.filename=model_kwargs['name'][0].split('.')[0]
        self.density = model_kwargs['high_res'].squeeze().numpy() # shape: (256, 256)
        a=np.sum(self.density)
        self.image = (Denormalize(model_kwargs['low_res'].squeeze().numpy())*255).astype(np.uint8).transpose(1,2,0)# 256*256*3
    ...
    def evaluate(self, samples, model_kwargs):
        ...
        self.recorded_mae=np.array(np.abs(pred_count-np.sum(self.crowd_count)))
        print(progress)
    ...
    def evaluate_sample(self, sample, index):
        ...
        pred_count = self.get_circle_count(sample, draw=DRAW_MAPS)
    ...
    def get_circle_count(self, image, threshold=0, draw=False, name=None):
        ...
        # Draw contours on GT density maps:
        gt = np.array(self.density[:, :, np.newaxis])
        ...
        for i in range(0, 255):
            for j in range(0, 255):
                ...
                jleft=max(0, j-1)
                jright=min(j+2, 256)
                neighborhood = gt[ileft:iright, jleft:jright ]
                # Calculate the maximum value in the neighborhood
                neighborhood_max = neighborhood.max()
                # Check if the current neighborhood has a higher maximum value
                if neighborhood_max <= gt[i,j] and gt[i,j]!=0:
                    contoursGT.append( (i,j) )

        if draw:
            canvas=np.array(self.image)
            ...
            for point in contours:
                ...
                cv2.drawMarker(canvas, (int(x), int(y)),  color=(0, 0, 255), markerSize = 2, )

            for point in contoursGT:
                y,x = point[0], point[1]
                cv2.drawMarker(canvas, (int(x), int(y)),  color=(0, 255, 0), markerSize = 3)

            threshedImg = np.repeat(threshedImg[:,:,np.newaxis], 3,-1)
            morphImg = np.repeat(morphImg[:,:,np.newaxis], 3,-1)
            gt=np.repeat(self.density[:,:,np.newaxis], 3,-1)
            gt=(gt*256).clip(0, 255)
            image = np.concatenate([gt, self.image, canvas, contoursImg, threshedImg, morphImg], axis=1)
            name=self.filename
            cv2.imwrite(f'experiments/WITHOUT/{name}_album.jpg', image)
