...
def main():
    ...
    for _ in os.listdir(args.data_dir):
        ...
        data_parameter = DataParameter(model_kwargs, args)  # model_kward corrupts here
        if 0 in model_kwargs['low_res'].shape:
            continue
        # model_kwargs['low_res'] = model_kwargs['low_res']#[:2]

        # model_kwargs['low_res']=model_kwargs['low_res'].to(dist_util.dev())

        # model_kwargs['high_res']=model_kwargs['high_res'].to(dist_util.dev())
        # model_kwargs = {k: v.to(dist_util.dev()) for k, v in model_kwargs.items()}
        # model_kwargs['low_res'] = model_kwargs['low_res'][:60]
        # print(model_kwargs['low_res'].shape)
        
        # Add some noise to prepared heatmap
        prepared_map=model_kwargs["prepared"]
        TIME=500
        tstep=th.ones(size=(model_kwargs['low_res'].shape[0],) )*TIME
        noised_prepared_map = diffusion.q_sample(prepared_map,tstep)
        # done
        ...
        while data_parameter.resample:
            ...
            samples = diffusion.p_sample_loop(
                model,
                (model_kwargs['low_res'].shape[0], args.pred_channels, model_kwargs['low_res'].shape[2],
                 model_kwargs['low_res'].shape[3]),
                clip_denoised=args.clip_denoised,
                model_kwargs=model_kwargs,
                noise=noised_prepared_map
            )
            samples, count = samples
            samples = samples["sample"]  # ”sample“就是采样的结果, 尺度是[Batch*Channel*H*W]，这里取得(1,1,256,256)
            # for iter in count:
            ...
            recorded_mae=data_parameter.recorded_mae

            count = np.sum(count, axis=1)
            gt_count = data_parameter.crowd_count.sum()
            if avg_mae is None:
                avg_mae = np.expand_dims(recorded_mae, axis=0)
            else:
                avg_mae = np.concatenate([avg_mae, np.expand_dims(recorded_mae, axis=0)], axis=0)
            count = [_ for _ in count]
            gt_count = [gt_count]
            gt_count.extend(count)
            # count = np.concatenate((np.asarray(gt_count), count))
            # print(gt_count[0])
            # print("Warning: the following chart records mae on the regression branch(not to be used in sampling) and is highly INACCURATE")
            print("Maes: ",avg_mae," Mean value:" ,np.mean(avg_mae, axis=0))
...
def get_circle_count(image, threshold=0, draw=False, name=None):
    ...
    th, threshedImg = cv2.threshold(denoisedImg, threshold, 255,
                                    cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)  # src, thresh, maxval, type
...
def create_crops(model_kwargs, args):
    ...
    model_kwargs['crowd_count'] = th.sum((model_kwargs['high_res'] + 1) * 0.5 * args.normalizer,
                                         dim=(1, 2, 3)).cpu().numpy()
...
def load_data_for_worker(base_samples, batch_size, normalizer, pred_channels, file_name, class_cond=False):
    print("base_samples ", base_samples)
    ...
    for file in img_list:
        ...
        #
        file = file.replace('test_den', 'test_prep').replace('csv', 'npy')
        prep_heatmap = np.load(file=file)
        prep_arr.append(prep_heatmap)

    rank = dist.get_rank()
    num_ranks = dist.get_world_size()
    buffer, den_buffer, prep_buffer = [], [], []
    label_buffer = []
    name_buffer = []
    print("len(image_arr) is ", len(image_arr))
    while True:
        for i in range(rank, len(image_arr), num_ranks):
            buffer.append(image_arr[i]), den_buffer.append(den_arr[i]), prep_buffer.append(prep_arr[i])
            ...
            if len(buffer) == batch_size:
                ...
                prep_batch = th.from_numpy(np.stack(prep_buffer)).float()
                # den_batch = den_batch / normalizer
                prep_batch = 2 * prep_batch - 1
                prep_batch = prep_batch.permute(0, 3, 1, 2)
...
def OLD_load_data_for_worker(base_samples, batch_size, normalizer, pred_channels, file_name, class_cond=False):
    # start = time()
        ...
        for i in range(rank, len(image_arr), num_ranks):
            ...
            if len(buffer) == batch_size:
                ...
                if class_cond:
                    res["y"] = th.from_numpy(np.stack(label_buffer))
                yield res
                buffer, label_buffer, name_buffer, den_buffer, prep_buffer = [], [], [], [], []
